...
class sep_obj:
    ...
    def separate_frames(self):
        ...
        ft = open('%s' % self.pimg.file_name, 'ab')
        ft.seek(self.pimg.header_length)
        ...
        #tracer_images,particle_images = pool.map(separation_alg.separation_alg,objList)
        particle_images = pool.map(separation_alg.separation_alg,objList)
        ...
        for f in range(0,f_tot):
            ...
            self.pimg.write_frame(np.flipud(particle_images[f,:,:]))
