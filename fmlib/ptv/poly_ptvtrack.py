...
class ptvtrack(ptvbase):
    ...
    def __init__(self,img_filename,    
        ...
        Requires:
           ...
           offset_x               - int   - Optional - offset for search radius in horizontal dir
           offset_y              - int   - Optional - offset for search radius in vertical dir          
    ...
    def gen_ptv_comment(self):
        ...
        return "%s\n%s %s %d %d %d %d %.1f %d %s %d %d %d %d %d %d %s %d\nfmlib git sha: #SHA#\n%s\n\n" % (getpass.getuser(), 
                                                                                 inspect.stack()[-1][1],
                                                                                 self.img.file_name,
                                                                                 self.start_frame,
                                                                                 self.track_increment,
    ...
    def track_all_frames(self):
        ...
        self.xgl2 = pframeB[:,0]
        self.ygl2 = pframeB[:,1]
        self.areas = pframeA[:,2]
        self.orientations = pframeA[:,3]
        self.majors = pframeA[:,4]
        self.minors = pframeA[:,5]
        
        self.areas2 = pframeB[:,2]
        self.orientations2 = pframeB[:,3]
        self.majors2 = pframeB[:,4]
        self.minors2 = pframeB[:,5]
        ...
        self.add_tracks([track(self.start_frame,self.xgl[s],self.ygl[s],self.areas[s],self.orientations[s],self.majors[s],self.minors[s],s) for s in range(len(self.xgl))])
        ...
        for t in self.tracks:
            if t is not False and t is not None:
                if (t.finished == 0) & ((t.current-1) > self.dontsave):
                    #self.ptv.write_track( t.start, np.column_stack( (t.x, t.y, t.area,t.orientation,t.major,t.minor, list(range(t.start,t.stop+1)) ) ) )
                    self.ptv.writre_track(self.tracks[i].start, np.column_stack( (self.tracks[i].x,self.tracks[i].y, self.tracks[i].area, self.tracks[i].orientation, self.tracks[i].major,self.tracks[i].minor,range(self.tracks[i].start,self.tracks[i].stop+1) ) ) )
                    extra_track +=1
    ...
    def track_frames1(self,frameA_index,frameB_index,debug=None):
        ...
        used = []
        xy2x = (np.repeat(self.xgl+self.offset_x+self.xgd,len(self.xgl2)) - np.tile(self.xgl2,len(self.xgl))).reshape(len(self.xgl),len(self.xgl2))
        xy2y = (np.repeat(self.ygl+self.offset_y+self.ygd,len(self.xgl2)) - np.tile(self.ygl2,len(self.xgl))).reshape(len(self.xgl),len(self.xgl2))
        xy2d = xy2x**2+xy2y**2
        lost = 0
        ...
        try:
            for k in range(len(self.xgl)):
                if np.min(xy2d) < self.srs:
                    ...
                    if x2i not in used:
                        self.tracks[xi].add_frame(self.xgl2[x2i],self.ygl2[x2i],self.areas2[x2i],self.orientations2[x2i],self.majors2[x2i],self.minors2[x2i],1)
                        used.append(x2i)
                        xy2d[:,x2i] = np.inf
                        xy2d[xi,:] = np.inf
                    else:
            ...
            new = [track(self.start_frame+self.track_increment,self.xgl2[i],self.ygl2[i],self.areas2[i],self.orientations2[i],self.majors2[i],self.minors2[i],1) for i in (np.setdiff1d(range(len(self.xgl2)), used))];
            ...
            sxyuv = np.zeros((len(self.active),9))
            for idx,i in enumerate(self.active):
                sxyuv[idx,:] = [ 1, self.tracks[i].x[-2], self.tracks[i].y[-2], self.tracks[i].u[-1], self.tracks[i].v[-1], self.tracks[i].area[-2],self.tracks[i].orientation[-2],self.tracks[i].major[-2],self.tracks[i].minor[-2]]
...
#        xy2d = xy2x**2+xy2y**2
        ...
        for j in self.active:
            ...
            if len(d):
                ...
                if d[ii] not in used:
                    # KEEP THEM
                    try:
                        self.tracks[j].add_frame(t[ii,0],t[ii,1],t[ii,2],t[ii,3],t[ii,4],t[ii,5],1)
                    except:
                        print(i)
        ...
        new = [track(frameA_index,self.xgl[z],self.ygl[z],self.areas[z],self.orientations[z],self.majors[z],self.minors[z],1) for z in (np.setdiff1d(range(len(self.xgl)), used))]
        
        saved = 0
        dropped = 0
        for i in finished:
            if (self.tracks[i].current-1) > self.dontsave:
                self.ptv.write_track( self.tracks[i].start, np.column_stack( (self.tracks[i].x,self.tracks[i].y, self.tracks[i].area, self.tracks[i].orientation, self.tracks[i].major,self.tracks[i].minor,range(self.tracks[i].start,self.tracks[i].stop+1) ) ) )
                saved += 1
            else:
                dropped += 1
...
def track_parse():
    ...
    parser.add_argument('offset_x',type=int,default=0,nargs='?', help='Optional - horizontal offset for search radius, default: 0')
    parser.add_argument('offset_y',type=int,default=0,nargs='?', help='Optional - horizontal offset for search radius, default: 0')
    ...
    print('img file: %s, start frame: %d, track increment: %d, no_of_pairs: %d, threshold: %d, lowpass: %.2f \n \
            ...
            particle position: %s, lsq bounds: %d \n' %(args.img_file,args.start_frame,args.track_increment,args.no_of_pairs,args.threshold,args.lowpass,args.highpass,args.peak_method,args.radius,args.search_radius,args.Ta,args.dontsave,args.centroid_method,n,args.lsq_bounds))
    print()"""
    fail = False
...
class track():
    ...
    '''
    def __init__(self,start_frame,x,y,area,orientation,major,minor,s):
        self.start = start_frame
        self.stop = start_frame
        self._x = np.zeros((100,))
        self._y = np.zeros((100,))
        self._area = np.zeros((100,))
        self._orientation = np.zeros((100,))
        self._major = np.zeros((100,))
        self._minor = np.zeros((100,))
    ...
    def add_frame(self, x,y,area,orientation,major,minor,s):
        ...
        if self.current == self.max:
            ...
            newdata = np.zeros((self.max,))
            newdata[:self.current] = self._area
            self._area = newdata

            newdata = np.zeros((self.max,))
            newdata[:self.current] = self._orientation
            self._orientation = newdata
            
            newdata = np.zeros((self.max,))
            newdata[:self.current] = self._major
            self._major = newdata
            
            newdata = np.zeros((self.max,))
            newdata[:self.current] = self._minor
            self._minor = newdata


        self._x[self.current] = x
        self._y[self.current] = y
        self._area[self.current] = area
        self._orientation[self.current] = orientation
        self._major[self.current] = major
        self._minor[self.current] = minor
    ...
    def end(self):
        ...
        self._area = np.array(self._area[:self.current])
        self._orientation = np.array(self._orientation[:self.current])
        self._major = np.array(self._major[:self.current])
        self._minor = np.array(self._minor[:self.current])
    ...
    @property
    def area(self):
        return self._area[:self.current]
    
    @property
    def orientation(self):
        return self._orientation[:self.current]
 
    @property
    def major(self):
        return self._major[:self.current]
    
    @property
    def minor(self):
        return self._minor[:self.current]
