...
class imgio: 
    ...
    def _read_header(self):
        ...
        with open("%s" % self.file_name, "rb") as f:
            ...
            f.readline()
            self.ix, self.iy, self.it = list(map(int, f.readline().split()))
            self.min, self.max = list(map(float, f.readline().split()))
            self.unsigned, self.bytes, self.channels = list(map(int, f.readline().split()))
    ...
    def read_frame(self,frame_number):
        ...
        with open("%s" % self.file_name, "rb") as f:
            ...
            byte = np.frombuffer(f.read(self.ix*self.iy*self.bytes),dtype=self.type) 
    ...
    def read_subframe(self,i,x_start,y_start,width,height):
        ...
        with open("%s" % self.file_name, "rb") as f:
            ...
            for j in np.arange(height):
                ...
                byte[j*width:(j+1)*width] = np.frombuffer(f.read(width),dtype=self.type)
            self.file_pos = f.tell()
