...
class pivio:
    ...
    def _read_header(self):
        ...
        with open("%s" % self.file_name, "rU") as f:
            ...
            f.readline()
            self.ix, self.iy = list(map(int, f.readline().split()))
            self.nx, self.ny, self.nt = list(map(int, f.readline().split()))
            self.x0, self.y0, self.t0 = list(map(int, f.readline().split()))
            self.dx, self.dy, self.dt = list(map(int, f.readline().split()))
            self.sx, self.sy, self.st = list(map(float, f.readline().split()))
            self.rx, self.ry, self.rt = list(map(float, f.readline().split()))
    ...
    def read_frame2d(self,frame_number,noflip=False):
        ...
        if noflip is True:
            frame2d = [frame[i].reshape(self.ny,self.nx) for i in range(self.cols)]
        else:
            frame2d = [np.flipud(frame[i].reshape(self.ny,self.nx)) for i in range(self.cols)]
            frame2d[2] = -frame2d[2]
