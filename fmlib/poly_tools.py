...
def peak_finder_com(img, radius,threshold=0):
    ...
    regions_filt = np.array(regions)[np.array([props.area<50 for props in regions],dtype='bool')]
    xy = np.zeros((len(regions_filt),6))
    f2 = np.copy(img)
    row = 0
    for i, dxy in enumerate(slices):
        slab = lab[dxy]
        region = regionprops(slab)
        if region[0].area < 50:
            area = region[0].area
            orientation = region[0].orientation
            major = region[0].major_axis_length
            minor = region[0].minor_axis_length        
            simg = np.copy(f2[dxy])
            simg[slab!=(i+1)] = 0
            c = measurements.center_of_mass(simg)
            xy[row,:] = dxy[1].start+np.round(c[1]),dxy[0].start+np.round(c[0]),area,orientation,major,minor
            row+=1
...
def peak_finder_com_subpixel(img, radius,threshold=0):
    ...
    slices = measurements.find_objects(lab)
    #regions = regionprops(lab) 
    #regions_filt = np.array(regions)[np.array([props.area<80 for props in regions],dtype='bool')]
    xy = np.zeros((len(slices),6))
    f2 = np.copy(img)
    row = 0
    for i, dxy in enumerate(slices):
        slab = lab[dxy]
        ...
        if region[0].area < 80:
            ...
            simg[slab!=(i+1)] = 0
            c = measurements.center_of_mass(simg)
            xy[row,:] = dxy[1].start+c[1],dxy[0].start+c[0],area,orientation,major,minor
        row+=1
    todel = np.where(np.sum(abs(xy),axis=1)==0)[0]
    xy = np.delete(xy,todel,axis=0)
    return xy
...
def peak_finder_maxfilter(img,radius,threshold=0):
    ...
    xy = np.zeros((len(slices),6),np.int)
    img2 = np.copy(img)
    row = 0
    for i,dxy in enumerate(slices):
        slab = labeled[dxy]
        ...
        xy[i,:2] =  x+dxy[1].start, y+dxy[0].start
        region = regionprops(simg)
        if region[0].area > 4:
            area = region[0].area
            orientation = region[0].orientation
            major = region[0].major_axis_length
            ...
            simg[slab!=(i+1)] = 0
            c = measurements.center_of_mass(simg)
            xy[row,2:] = area,orientation,major,minor
            row+=1
...
def registration_array_3pt(img,xy,radius=PARTICLE_RADIUS,lsq_bounds=PARTICLE_LSQ_BOUNDS):
    ...
    sxy[sx,0] += (0.5*(NL[img[xy[sx,1],xy[sx,0]-1]]).astype('float')-(NL[img[xy[sx,1],xy[sx,0]+1]])/dx[sx]).astype('float')
    sxy[sy,1] += (0.5*(NL[img[xy[sy,1]-1,xy[sy,0]]]).astype('float')-(NL[img[xy[sy,1]+1,xy[sy,0]]])/dy[sy]).astype('float')
